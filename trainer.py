...
def compute_loss_and_accuracy(
    ...
    with torch.no_grad():
        for (X_batch, Y_batch) in dataloader:
            #Y_batch = torch.nn.functional.one_hot(Y_batch, num_classes=10)
            ...
            number_of_predictions += X_batch.shape[0]
            predicted = output_probs.argmax(dim = 1).to(torch.int)

            number_of_accurate += torch.sum((predicted == Y_batch).to(int))
    
    average_loss = loss/ number_of_steps
    accuracy = number_of_accurate/number_of_predictions
